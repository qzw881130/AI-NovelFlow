// 繁体中文任务队列翻译 - 继承自简体中文
import zhCN from '../zh-CN/tasks';

export default {
  tasks: {
    ...zhCN.tasks,
    title: '任務佇列',
    subtitle: '檢視和管理生成任務',
    allTasks: '全部任務',
    allTypes: '全部類型',
    pending: '等待中',
    running: '執行中',
    completed: '已完成',
    failed: '失敗',
    error: '錯誤',
    comfyuiHint: 'ComfyUI 任務異常時，可查看 Clip 狀態並重新生成缺失 Clip。',
    cancelled: '已取消',
    taskId: '任務ID',
    taskType: '任務類型',
    status: '狀態',
    progress: '進度',
    createdAt: '建立時間',
    completedAt: '完成時間',
    actions: '操作',
    cancel: '取消',
    retry: '重試',
    delete: '刪除',
    viewLogs: '檢視日誌',
    viewWorkflow: '檢視工作流',
    noTasks: '暫無任務',
    noTasksCreated: '尚未建立任何任務',
    noTasksInStatus: '目前篩選條件下無任務',
    clearCompleted: '清空已完成',
    confirmClear: '確定要清空所有已完成的任務嗎？',
    terminateAll: '全部終止',
    systemStatusTitle: '系統狀態',
    connecting: '正在連接...',
    online: '線上',
    offline: '離線',
    gpuModel: '顯卡型號',
    gpuUsage: 'GPU 使用率',
    realtime: '即時',
    vramUsage: '顯存佔用',
    ramUsage: '記憶體佔用',
    gpuTemperature: 'GPU 溫度',
    queueTasks: '佇列任務',
    taskList: '任務列表',
    multiClipDetails: 'Multi-Clip 詳情',
    clipCount: '{count} 個 Clip',
    dialogueCount: '台詞 {count}',
    promptId: 'prompt_id: {id}',
    viewClipWorkflow: '檢視 Clip 實際工作流',
    viewClipVideo: '檢視 Clip 影片',
    clipStatuses: {
      pending: '等待中',
      promptBuilding: '構建 H3 提示詞',
      queued: '生成中',
      running: '執行中',
      succeeded: '已完成',
      failed: '失敗',
      cancelled: '已取消',
    },
    referenceImages: '參考圖',
    referenceImage: '參考圖',
    types: {
      characterPortrait: '人設生成',
      characterVoice: '生成音色',
      audioEventTts: 'Audio Event TTS',
      audioPrepare: 'AudioDrive 音訊準備',
      characterAudio: '角色音訊',
      narratorAudio: '旁白音訊',
      sceneImage: '場景圖片',
      shotImage: '分鏡圖片',
      shotImageBatch: '批次分鏡圖片',
      keyframeImage: '關鍵幀圖片',
      singleImageEdit: '單圖編輯',
      shotVideo: '分鏡影片',
      shotVideoBatch: '批次分鏡影片',
      chapterVideo: '章節影片',
      transitionVideo: '轉場影片',
      propImage: '道具圖片',
    },
    systemDefault: '系統預設',
    custom: '自訂',
    workflowNames: {
      '系统默认-人设生成': '系統預設-人設生成',
      '系统默认-场景生成': '系統預設-場景生成',
      '系统默认-分镜生图': '系統預設-分鏡生圖',
      '系统默认-分镜生视频': '系統預設-單幀生影片',
      '系统默认-单帧生视频': '系統預設-單幀生影片',
      '系统默认-转场视频': '系統預設-轉場影片',
      '系统默认-音色设计': '系統預設-音色設計',
      '系统默认-音频生成': '系統預設-音訊生成',
      'Z-image-turbo 单图生成': 'Z-image-turbo 單圖生成',
      'Flux2-Klein-9B 分镜生图': 'Flux2-Klein-9B 分鏡生圖',
      'Flux2-Klein-9B 关键帧生图': 'Flux2-Klein-9B 關鍵幀生圖',
      'LTX2 视频生成-直接版': 'LTX2 影片生成-直接版',
      'LTX2 视频生成-扩写版': 'LTX2 影片生成-擴寫版',
      'Minimax H3 首尾帧生视频': 'Minimax H3 首尾幀生影片',
      'Minimax H3 三帧生视频': 'Minimax H3 三幀生影片',
      'Minimax H3 四帧生视频': 'Minimax H3 四幀生影片',
      'LTX2 镜头转场视频': 'LTX2 鏡頭轉場影片',
      'LTX2 光线转场视频': 'LTX2 光線轉場影片',
      'LTX2 遮挡转场视频': 'LTX2 遮擋轉場影片',
      'Z-image-turbo 场景生成': 'Z-image-turbo 場景生成',
      'Flux2-Klein-9B 分镜生图双图参考': 'Flux2-Klein-9B 分鏡生圖雙圖參考',
    },
    workflowDescriptions: {
      '系统预设的人设生成工作流': '系統預設的人設生成工作流',
      '系统预设的人设生成工作流（Flux2 Klein 9B 三视图）': '系統預設的人設生成工作流（Flux2 Klein 9B 三視圖）',
      '系统预设的场景生成工作流': '系統預設的場景生成工作流',
      '系统预设的音色设计工作流': '系統預設的音色設計工作流',
      '基于文本提示词设计音色，生成语音': '基於文字提示詞設計音色，生成語音',
      '基于参考音频生成语音，支持情感提示词控制': '基於參考音訊生成語音，支援情感提示詞控制',
      'Z-image-turbo 场景生成工作流': 'Z-image-turbo 場景生成工作流',
      'Z-image-turbo【非三视图】': 'Z-image-turbo【非三視圖】',
      'Flux2-Klein-9B 图像编辑工作流，支持角色参考图': 'Flux2-Klein-9B 圖像編輯工作流，支援角色參考圖',
      'Flux2-Klein-9B 图像编辑工作流，仅支持角色参考图': 'Flux2-Klein-9B 圖像編輯工作流，僅支援角色參考圖',
      'Flux2-Klein-9B 双图参考工作流，支持角色参考图+场景参考图，保持场景一致性': 'Flux2-Klein-9B 雙圖參考工作流，支援角色參考圖+場景參考圖，保持場景一致性',
      'Flux2-Klein-9B 关键帧生图工作流，支持参考图': 'Flux2-Klein-9B 關鍵幀生圖工作流，支援參考圖',
      'LTX-2 图生视频，直接使用用户提示词': 'LTX-2 圖生影片，直接使用使用者提示詞',
      'LTX-2 图生视频，使用 Qwen3 自动扩写提示词': 'LTX-2 圖生影片，使用 Qwen3 自動擴寫提示詞',
      '适合：首尾帧是同一场景不同景别/角度': '適合：首尾幀是同一場景不同景別/角度',
      '适合：首尾帧颜色差很多，但场景/人物不变': '適合：首尾幀顏色差很多，但場景/人物不變',
      '适合：两张图差异大，想自然衔接': '適合：兩張圖差異大，想自然銜接',
    },
    workflowUsed: '使用的工作流程',
    taskNames: {
      characterPortrait: '生成角色形象: {name}',
      sceneImage: '生成場景圖: {name}',
      shotImage: '生成分鏡圖片: {name}',
      shotVideo: '生成分鏡影片: {name}',
      transitionVideo: '生成轉場影片: {from} → {to}',
      chapterVideo: '生成章節影片: {name}',
    },
    taskDescriptions: {
      characterPortrait: "為角色 '{name}' 生成人設圖",
      sceneImage: "為場景 '{name}' 生成圖片",
      shotImage: "為分鏡 '{name}' 生成圖片",
      shotVideo: "為分鏡 '{name}' 生成影片",
      transitionVideo: "生成從分鏡 {from} 到 {to} 的轉場影片",
      chapterVideo: "為章節 '{name}' 生成影片",
    },
    workflowDetails: '工作流詳情',
    generationPrompt: '生成提示詞',
    workflowJSON: '工作流 JSON',
  },
};
