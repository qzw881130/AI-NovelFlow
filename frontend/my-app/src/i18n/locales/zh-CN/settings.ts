...
export default {
  ...
  systemSettings: {
    ...
    timeout: '请求超时',
    timeoutUnit: '分钟',
    timeoutDesc: '单次 LLM 请求的最大等待时间，默认 30 分钟',
    testConnection: '测试连接',
    ...
    workflow: {
      ...
      scene: '场景生成',
      shotScene: '分镜图生成（场景）',
      shotCharacterScene: '分镜图生成（角色+场景）',
      shotSceneProp: '分镜图生成（场景+道具）',
      shot: '分镜图生成（角色+场景+道具）',
      video: '单帧生视频',
  ...
  promptConfig: {
    ...
    newPrompt: '新建提示词',
    exportAllPrompts: '打包所有提示词',
    exportingAll: '打包中...',
    exportAllSuccess: '提示词包已开始下载',
    exportAllFailed: '打包下载提示词失败',
    ...
    types: {
      ...
      assetIdentityResolution: '已有角色归一化提示词',
      assetIdentityResolutionDesc: '将章回候选归一到已有角色；正文维护于本地 existing_asset_resolver.json。',
      characterAppearanceGeneration: '角色换装提示词',
      characterAppearanceGenerationDesc: '基于正式角色基图构建章回换装编辑；正文维护于本地 character_appearance_edit.json。',
      ...
      shotContractRepair: '分镜契约自动修复提示词',
      shotContractRepairDesc: '根据程序生成的结构化 Repair Context 定向修复可自动处理的分镜契约错误',
      shotImagePrompt: '主分镜图生成提示词构建',
      shotImagePromptDesc: '用于根据 Shot 和参考资产构建主分镜图生图提示词',
      videoModeRecommender: '视频生成模式推荐',
      videoModeRecommenderDesc: '用于为 Shot 推荐单帧、首尾帧或多关键帧视频生成模式',
      keyframeDescription: '关键帧描述提示词',
      keyframeDescriptionDesc: '用于生成分镜关键帧描述的提示词模板',
      keyframePlanner: '关键帧时间轴规划',
    ...
    placeholderTipStyle: '直接填写渲染风格描述，不使用变量或 JSON',
    noTemplates: '暂无提示词模板',
    ...
    tipStyle: '仅描述媒介、线条、色彩、材质和光影；保留角色、道具和世界观，不指定镜头、动作或对白。',
    charCount: '提示词模板字符数',
    ...
    templateNames: {
      ...
      '标准角色解析': '标准角色解析',
      '章节分镜导演解析': '章节分镜导演解析',
      '分镜契约自动修复 V1': '分镜契约自动修复 V1',
      '主分镜图生成提示词构建': '主分镜图生成提示词构建',
      '视频生成模式推荐': '视频生成模式推荐',
      '关键帧时间轴规划': '关键帧时间轴规划',
      '视频关键帧生图提示词构建': '视频关键帧生图提示词构建',
      '关键帧过渡规划': '关键帧过渡规划',
      'MiniMax H3 单帧视频提示词构建': 'MiniMax H3 单帧视频提示词构建',
      'MiniMax H3 首尾帧视频提示词构建': 'MiniMax H3 首尾帧视频提示词构建',
    ...
    templateDescriptions: {
      ...
      'Q版卡通': '可爱Q版卡通风格',
      '水墨风格': '毛笔墨韵、浓淡晕染与宣纸肌理，以墨色和克制淡彩统一画面',
      '标准分镜拆分': '适用于大多数小说的标准分镜拆分',
      '电影风格分镜': '电影级分镜拆分，强调画面构图和镜头语言',
      '动漫风格': '二维动漫渲染，清晰线稿、赛璐璐分层阴影与统一色彩',
      'Q版风格': '稳定头身比的二维Q版角色，场景和道具保持原有结构与功能比例',
      '3D动画风格': '风格化三维动画质感，清晰体积、柔和光照与统一材质表现',
      '水彩绘本风格': '透明水彩叠色、柔和边缘与纸张颗粒，呈现手绘绘本质感',
      '油画风格': '油彩笔触、层叠罩染与画布肌理，强调色彩和明暗塑形',
      '美式漫画风格': '美式漫画墨线、鲜明色块、排线与半色调网点表现',
      '像素艺术风格': '统一像素网格、有限色板与清晰像素簇，避免模糊和平滑渐变',
      '剪纸艺术风格': '剪纸轮廓、层叠色纸与轻微投影，保留主体轮廓和辨识细节',
      '黏土定格风格': '手工黏土定格质感，柔和体积、细微塑形痕迹与哑光表面',
      '国风工笔风格': '工笔细线勾勒、层层设色与绢纸质感，细致保留服饰和器物特征',
      ...
      '章节分镜导演解析': '根据章节正文和角色/场景/道具白名单规划导演 Shot 数据',
      '分镜契约自动修复 V1': '按程序生成的结构化 Repair Context 定向修复可自动处理的分镜契约错误',
      '主分镜图生成提示词构建': '根据 Shot 数据和正式参考资产构建 Qwen-Image-Edit 主分镜图提示词',
      '视频生成模式推荐': '根据 Shot 与 Workflow 能力推荐 SINGLE_FRAME / FIRST_LAST_FRAME / MULTI_KEYFRAME',
      '关键帧时间轴规划': '为 FIRST_LAST_FRAME 或 MULTI_KEYFRAME Shot 规划关键帧时间轴',
      '视频关键帧生图提示词构建': '根据上一关键帧、主分镜图和参考资产构建下一关键帧生图提示词',
      '关键帧过渡规划': '根据相邻关键帧规划自然过渡的 Segment 动态导演描述',
      'MiniMax H3 单帧视频提示词构建': '根据单张权威起始帧、Clip 动态意图和精确对白构建 H3 单帧视频提示词',
      'MiniMax H3 首尾帧视频提示词构建': '根据起止两张权威时间帧、过渡描述和精确对白构建 H3 首尾帧视频提示词',
      'MiniMax H3 多关键帧视频提示词构建': '根据多张时间顺序关键帧、过渡 Segment 和精确对白构建 H3 多关键帧提示词',
    },
  },
