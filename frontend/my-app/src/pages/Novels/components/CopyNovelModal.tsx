...
export function CopyNovelModal({ novels, onClose, onCopy }: CopyNovelModalProps) {
  ...
  const handleSourceChange = (id: string) => {
    ...
    setTitle(selected ? t('novels.copyTitleDefault', { title: selected.title }) : '');
    setError('');
  };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
        <h2 id="copy-novel-heading" className="text-lg font-semibold text-gray-900">{t('novels.copyNovel')}</h2>
        <button type="button" onClick={onClose} disabled={isCopying} aria-label={t('common.close')} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 disabled:opacity-50">
          <X className="h-5 w-5" aria-hidden="true" />
        </button>
      </div>
      <form onSubmit={handleSubmit} aria-busy={isCopying}>
        <div className="space-y-4 px-5 py-5">
          <p id="copy-novel-scope" className="rounded-lg bg-blue-50 p-3 text-sm text-blue-800">{t('novels.copyScopeHint')}</p>
          <div>
            <label htmlFor="copy-novel-source" className="block text-sm font-medium text-gray-700">{t('novels.copySource')}</label>
            <select id="copy-novel-source" ref={sourceRef} required value={sourceId} onChange={(event) => handleSourceChange(event.target.value)} disabled={isCopying} className="input-field mt-1 min-w-0">
              <option value="">{t('novels.copySourcePlaceholder')}</option>
              {novels.map((novel) => <option key={novel.id} value={novel.id}>{novel.title}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="copy-novel-title" className="block text-sm font-medium text-gray-700">{t('novels.copyTitle')}</label>
            <input id="copy-novel-title" type="text" required value={title} onChange={(event) => setTitle(event.target.value)} disabled={!source || isCopying} className="input-field mt-1" />
          </div>
        ...
        <div className="flex flex-wrap justify-end gap-3 border-t border-gray-100 px-5 py-4">
          <button type="button" onClick={onClose} disabled={isCopying} className="btn-secondary">{t('common.cancel')}</button>
