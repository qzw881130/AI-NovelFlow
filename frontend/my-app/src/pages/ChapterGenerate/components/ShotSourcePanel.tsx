  ...
  return <><ChapterGovernancePanel novelId={novelId} chapterId={chapterId}/><details className="border rounded-lg p-3 bg-white mb-3 min-w-0 flex-shrink-0" open>
    ...
    <div className="space-y-2 mt-2 text-sm max-h-72 overflow-y-auto min-w-0">
      ...
      <div className="border rounded p-2 bg-slate-50"><p className="font-medium">Chapter Completion Manifest</p>
        ...
        <button className="btn-primary text-xs mt-2" disabled={!completionReadiness?.ready||!completionReadiness.manifestHash} onClick={async()=>{if(!completionReadiness?.manifestHash)return;const failed=completionReadiness.latestTask?.status==='failed'?completionReadiness.latestTask.taskId:undefined;const response=await chapterShotSplitsApi.complete(novelId,chapterId,completionReadiness.manifestHash,failed);setActionMessage(response.success?(failed?'章回完整交付显式重试已提交':'章回完整交付任务已提交'):String(response.message));void load();}}>{completionReadiness?.latestTask?.status==='failed'?'显式重试章回完整交付':'按封存 Manifest 完成章回'}</button>
